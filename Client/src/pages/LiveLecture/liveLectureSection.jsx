...
import { Calendar, Clock, Users, Video, Edit, Trash2, ExternalLink, Search, Filter, ChevronLeft, ChevronRight } from 'lucide-react';
import {
    ...
    const renderLectureActions = (lecture) => {
        ...
        return (
            <MyCourseComponents.Button
                ...
                disabled={status === "completed" || (status === "scheduled" && !isLectureStartable(lecture))}
            >
                <Video className="w-4 h-4 mr-2" />
                {status === "ongoing" ? 'Join Lecture' : 
                 status === "completed" ? 'Lecture Ended' : 
                 `Starts in ${calculateTimeRemaining(lecture)}`}
            </MyCourseComponents.Button>
        );
    };
            ...
            <div className="max-w-7xl mx-auto">
                <header className="mb-8">
                    ...
                    <p className="text-gray-600">
                        {user.isInstructor ? 'Manage your live lectures' : 'Your Live lectures'}
                    </p>
                </header>
                        ...
                        currentLectures.map((lecture) => (
                            <div key={lecture._id} 
                                ...
                                ${lecture.status === 'ongoing' ? 'border-l-4 border-green-500' : 
                                  lecture.status === 'completed' ? 'border-l-4 border-gray-400' : ''}`}
                            >
                                <div className="p-6">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <div className="flex items-center gap-2 mb-2">
                                                <h3 className="text-xl font-semibold">Title: {lecture.title}</h3>
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusBadgeClass(lecture.status)}`}>
                                                    {lecture.status.charAt(0).toUpperCase() + lecture.status.slice(1)}
                                                </span>
                                            </div>
                                            <p className="text-gray-600 mb-4">Course: {lecture.course.title}</p>
                                    ...
                                    <p className="text-gray-600 mb-4">Description: {lecture.description}</p>
                                </div>
                            </div>
                        ))
                    ) : (
